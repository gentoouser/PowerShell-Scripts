...
#>
PARAM (
    [Parameter(Mandatory=$false,HelpMessage="Folder path to Archived homedrive.")][string]$Archive = "\\unc\MailBox_Export",
    [Parameter(Mandatory=$false,HelpMessage="Exchange Server.")][string]$Server = "exchange",
...
Function Export-Mail {
	...
	( 
		...
		[Parameter(Mandatory=$false,Position=5,HelpMessage="Disable user in exchange if PST export exists.")][switch]$ExportedDisable,
		[Parameter(Mandatory=$false,Position=6,HelpMessage="Set the priority of export.")][ValidateSet("Normal","High")][string]$Priority="Normal",
		[Parameter(Mandatory=$false,Position=7,HelpMessage="Export only specific year.")][int64]$Year,
		[Parameter(Mandatory=$false,Position=8,HelpMessage="Array of Active statuses to look for.")][array]$GoodStatuses = @("CompletionInProgress","InProgress","Queued","Retrying"), 
		[Parameter(Mandatory=$false,Position=9,HelpMessage="Array of InActive statuses to look for.")][array]$BadStatuses = @("AutoSuspended","CompletedWithWarning","Failed","Suspended","Synced"),
		[Parameter(Mandatory=$false,Position=10,HelpMessage="Specifies the parent activity of the current activity")][int]$ParentId = -1,
		[Parameter(Mandatory=$false,Position=11,HelpMessage="Use following Credential for commands (Get-Credential)")][pscredential]$Credential,
		[Parameter(Mandatory=$false,Position=12,HelpMessage="Seconds to wait before refreshing progress.")][int]$Wait = 15
	)
				...
				If($Credential) {
					...
					If (-Not (Test-Path "PSHome:\") -or (Get-PSdrive -name "PSHome").root -ne $Archive) {
						...
						if ($PST) {
							...
							If ($ExportedDisable -and $Year -eq (get-date).year) {
			...
			If (($EJSValue -ne "Completed") -And ($EJSValue -ne "Failed")) {
				...
				If ($ExportJobStatusName -and $ExportJobStatus) {
					...
					while (($EJSValue -ne "Completed") -And ($EJSValue -ne "Failed")) {
						...
						If ($ExportJobStatusGuid) {
							#write-host ("`tExportRequestGuid: " + $ExportJobStatusGuid)
							$ExportJobStatus = Get-MailboxExportRequestStatistics -Identity $ExportJobStatusGuid
						}Else {
							#write-host ("`tExportJobStatusName: " + $ExportJobStatusName)
							$ExportJobStatus = $ExportJobStatusName | Get-MailboxExportRequestStatistics
						}
						#Fall Back
						...
						$EJSValue = $null
						#Use Diffrent calls depending on diffrent Exchange Verssions
						If ([int]$ExchangeVersion -gt 14) {
							$EJSValue = $ExportJobStatus.status.value
					...
					If ($CMYearSplit) {
						...
						while($YearCounter -le $CurrentYear) {
							...
							If ($YearCounter -eq (get-date).year ) {
								Export-Mail -User $DA.SamAccountName -Archive ($Archive  + "\" + $DA.SamAccountName) -Disable:$Disable -Priority $Priority -Wait $Wait -Year $YearCounter -ParentID 1 -ExportedDisable:$ExportedDisable -DisableLitigationHold:$DisableLitigationHold
								...
								Export-Mail -User $DA.SamAccountName -Archive ($Archive  + "\" + $DA.SamAccountName) -Priority $Priority -Wait $Wait -Year $YearCounter -ParentID 1 -DisableLitigationHold:$DisableLitigationHold
							}
							$YearCounter++
						}
					}Else{
						#Start Mail Export
						Export-Mail -User $DA.SamAccountName -Archive ($Archive  + "\" + $DA.SamAccountName) -Disable:$Disable -Priority $Priority -Wait $Wait -ParentID 1 -DisableLitigationHold:$DisableLitigationHold -ExportedDisable:$ExportedDisable
				...
				If ($CMYearSplit) {
					...
					while($YearCounter -le ((get-date).year)) {
						If((get-date).year -eq $YearCounter){
								Export-Mail -User $DA.SamAccountName -Archive ($Archive  + "\" + $DA.SamAccountName) -Disable:$Disable -Priority $Priority -Wait $Wait -Year $YearCounter -ParentID 2 -ExportedDisable:$ExportedDisable -DisableLitigationHold:$DisableLitigationHold
						}Else{
							Export-Mail -User $DA.SamAccountName -Archive ($Archive  + "\" + $DA.SamAccountName) -Priority $Priority -Wait $Wait -Year $YearCounter -ParentID 2 -DisableLitigationHold:$DisableLitigationHold
						}
						$YearCounter++
					}
				}Else{
					#Start Mail Export
					Export-Mail -User $DA.SamAccountName -Archive ($Archive  + "\" + $DA.SamAccountName) -Disable:$Disable -Priority $Priority -Wait $Wait -ParentID 1 -ExportedDisable:$ExportedDisable -DisableLitigationHold:$DisableLitigationHold
