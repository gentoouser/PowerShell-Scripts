...
.DESCRIPTION
  ...
  Model,Serial Number, CPU, RAM, Disks, Operating System, Sound Devices and Graphics Card Controller.
...
$UNCPSExecPath = "\\github.com\PSTools\PsExec.exe"
$RemoveServices=@(
...
$winrmcommands=@(
    'netsh advfirewall firewall add rule dir=in name="DCOM" program=%systemroot%\system32\svchost.exe service=rpcss action=allow protocol=TCP localport=135 remoteip=10.100.0.0/16,10.200.204.0/24,172.16.242.0/24,LocalSubnet profile=any',
    'netsh advfirewall firewall add rule dir=in name="WMI-In" program=%systemroot%\system32\svchost.exe service=winmgmt action = allow protocol=TCP localport=any remoteip=10.100.0.0/16,10.200.204.0/24,172.16.242.0/24,LocalSubnet profile=any',
    'netsh advfirewall firewall add rule dir=in name="UnsecApp" program=%systemroot%\system32\wbem\unsecapp.exe action=allow remoteip=10.100.0.0/16,10.200.204.0/24,172.16.242.0/24,LocalSubnet profile=any',
...
}Elseif(Test-Path ($env:ProgramFiles + "\Sysinternals Suite\PsExec.exe")) {
    $PSExecPath = ($env:ProgramFiles + "\Sysinternals Suite\PsExec.exe")
...
}Elseif(Test-Path (${env:ProgramFiles(x86)} + "\Sysinternals Suite\PsExec.exe")) {
    $PSExecPath = (${env:ProgramFiles(x86)} + "\Sysinternals Suite\PsExec.exe")
}Else{
    $PSExecPath = $UNCPSExecPath
}
#endregion PSExecPath
write-host "Getting AD Computer Information"
...
Foreach ($ComputerName in $AllComputersNames) {
        Write-Progress  -Id 0 -Activity ("Processing Computer") -Status ("( " + $count + "\" + $AllComputersNamesCount + "): " + $CurrentComputer) -percentComplete ($count / $AllComputersNamesCount*100)
             #Create New Object
             ...
             If ($ComputerOS.Enabled -eq $true -and $null -ne $ComputerInfo.'DNS IP') {
                 ...
                 if ((-Not $psSession -or $psSession.Availability -ne "Available" ) -and $SetupWinRM) {                   
                     ForEach ($Command in $winrmcommands) {
                         Start-PSExec -Computer $ComputerName -Command $Command -PSExecPath $PSExecPath -maximumRuntimeSeconds 600
                     }
                     $psSession =  New-PSSession -UseSSL -Authentication Kerberos -ComputerName $ComputerName -ErrorAction SilentlyContinue -SessionOption $psSessionoptions
                     if (-Not $psSession -or $psSession.Availability -ne "Available" ) {
                         $psSession =  New-PSSession -ComputerName $ComputerName -ErrorAction SilentlyContinue 
                 ...
                 If ($cimSession -and $psSession) {
                     ...
                     #endregion NIC
                     $ComputerHW = Get-CimInstance -Class Win32_ComputerSystem -CimSession $cimSession |
                     ...
                     $ComputerCPU = Get-CimInstance win32_processor -CimSession $cimSession |
